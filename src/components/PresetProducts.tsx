...
const PRESETS: PresetItem[] = [
  {
    ...
    bgColor: "bg-amber-50 hover:border-amber-400",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" width="100%" height="100%">
      <rect width="100%" height="100%" fill="#111827" rx="30"/>
      <!-- Glowing grids -->
      ...
      <!-- Tech circuit path -->
      <path d="M 80,120 Q 150,150 200,130 T 320,160" fill="none" stroke="#fbbf24" stroke-width="2" stroke-dasharray="5,5" opacity="0.6" />
      <circle cx="80" cy="120" r="4" fill="#fbbf24" />
      <circle cx="320" cy="160" r="4" fill="#fbbf24" />
      ...
      <!-- Outer text -->
      <text x="200" y="360" font-family="'JetBrains Mono', monospace" font-size="20" fill="#fbbf24" font-weight="bold" text-anchor="middle" letter-spacing="4">NANO BANANA v2.5</text>
  ...
  },
  {
    ...
    bgColor: "bg-cyan-50 hover:border-cyan-400",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" width="100%" height="100%">
  ...
  },
  {
    ...
    bgColor: "bg-rose-50 hover:border-rose-400",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" width="100%" height="100%">
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm shadow-gray-150/40">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-bold text-gray-800">무료 브랜드 프리셋 에셋 선택</h3>
      </div>
      <p className="text-xs text-gray-500 mb-5 leading-relaxed">
        업로드할 이미지가 없으신가요? 나노 바나나가 준비한 독보적인 메카닉과 신스웨이브 시각 테마 벡터 에셋을 활용해 목업을 즉시 디자인해보세요!
      </p>
        ...
        {PRESETS.map((preset) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col text-left rounded-2xl border-2 p-3 transition-all duration-300 cursor-pointer ${preset.bgColor} ${
                isSelected
                  ? "border-gray-900 bg-white ring-4 ring-amber-100/50 scale-[1.02]"
                  : "border-gray-100"
              }`}
            ...
            >
              ...
              <div
                className="w-full aspect-square rounded-xl overflow-hidden mb-3.5 bg-gray-950 border border-gray-100 shadow-sm relative"
                dangerouslySetInnerHTML={{ __html: preset.svg }}
              />

              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-extrabold text-gray-900">{preset.name}</span>
                {preset.id === "nano-banana" && (
                  <span className="text-[9px] bg-amber-400 font-bold text-amber-950 px-1 py-0.5 rounded">🍌 추천</span>
                )}
              </div>
